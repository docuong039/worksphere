...
| **Mô tả** | Các chức năng quản lý người dùng hệ thống |
| **Số Use Cases** | 4 |
| **Actors** | Administrator, User (tự quản lý) |
| **Quyền yêu cầu** | `isAdministrator = true` (hoặc tự sửa của mình) |
| **Source Files** | `src/app/api/users/route.ts`, `src/app/api/users/[id]/route.ts` |
...
#### 10. Ghi chú
- Mật khẩu được mã hóa bằng bcrypt với salt rounds = 10 (`[id]/route.ts` Line 83).
- Email unique constraint do Prisma tự động kiểm tra.
...
#### 1. Mô tả
Use Case này cho phép chỉnh sửa thông tin người dùng. Người dùng có thể tự sửa thông tin cá nhân của mình (tên, email, mật khẩu). Quản trị viên có thể sửa tất cả thông tin bao gồm quyền admin và trạng thái.

#### 2. Tác nhân chính
- **Administrator**: Quản trị viên hệ thống (sửa tất cả).
- **User**: Người dùng tự sửa thông tin của mình.
...
#### 10. Ghi chú
- Người dùng thường có thể tự cập nhật thông tin cá nhân của mình (tên, email, password).
- Khi user tự sửa: hệ thống tự động loại bỏ `isAdministrator` và `isActive` khỏi data (Line 80-83).
